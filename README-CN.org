...
** MariaDB/MySQL to Elasticsearch
  ...
  基于 jdk 8 和 spring boot 2, 支持 Elasticsearch 7.6.2
...
config:
 ...
      increment-column: id       # *** 必须设定. 表示用来做数据增量操作时用, 一般使用自增 id 或 time(更新时间戳)
      ...
      delete: true               # 可以不设定. 是否定时删除已经标记为已删除的数据, 默认是 false
      delete-field: deleted      # 可以不设定. 批量删除时的字段名. 如表字段中 isDeleted
      delete-value: 1            # 可以不设定. 批量删除时的值. 如 1
#+END_SRC
