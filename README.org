...
** MariaDB/MySQL to Elasticsearch
   ...
   Based on jdk 8 and spring boot 2, support Elasticsearch 7.6.2
...
config:
 ...
      increment-column: id       # *** Must set. Indicates that it is used for data increment operations. Generally, it uses auto increment ~id~ or ~time~
      ...
      delete: true               # Whether to regularly delete data that has been marked as deleted, the default is false
      delete-field: deleted      # Field name when deleting in bulk. For example: isDeleted
      delete-value: 1            # The value when deleting in bulk. For example: 1
#+END_SRC
